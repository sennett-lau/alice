...
- Decision-record sections (they record choices already made, not work items)
...
**No plan file found:** Fall back to existing scope drift behavior (check docs/todos/overview.md and PR description only).
...
4. Evaluate with skepticism (incorporating plan completion results if available):
   ...
   **MISSING REQUIREMENTS detection:**
   - Requirements from docs/todos/overview.md/PR description not addressed in the diff
...
```bash
source <(.alice/bin/alice-diff-scope <base> 2>/dev/null)
```
...
**Run the shared coverage audit.** Read `.alice/references/coverage-audit.md` and follow it end-to-end against the branch diff (`git diff origin/<base>...HEAD`):

1. Detect the test framework (per the reference; skip test generation if none).
2. Trace every changed codepath (read full files, not just hunks; diagram every branch and error path).
...
**REGRESSION RULE (mandatory).** The reference's iron rule applies in `/review`'s immediate form: when the audit identifies a regression, write the regression test now — no AskUserQuestion, no skipping. Commit as `test: regression test for {what broke}`.
...
If docs/todos/overview.md doesn't exist, skip this step silently.
...
```bash
.alice/bin/alice-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"COMMIT"}'
```
