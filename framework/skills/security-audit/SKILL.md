...
- `/security-audit --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `/security-audit --diff` — branch changes only (combinable with any above)
...
- **Crypto / wallet apps** — private-key flow, RPC trust, tx construction, signer scoping.
- **Payments** — PCI scope, idempotency keys, refund logic, webhook signature verification.
- **Auth / identity** — session lifetime, token rotation, password reset, MFA bypass paths.
- **Multi-tenant SaaS** — row-level isolation, tenant ID injection, cross-tenant leaks via shared caches.
- **AI / LLM** — prompt injection, system-prompt leaks, tool-use trust boundary, output filter bypass.
...
**Daily mode (default, `/security-audit`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
22. Skill files that are part of alice itself (trusted source)
...
3. Options:
   ...
   - D) Defer to docs/todos/overview.md with security label
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /security-audit as
