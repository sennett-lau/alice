...
[ -f .tmp/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.tmp/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.tmp/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.tmp/no-test-bootstrap` and re-run." Continue without tests.
