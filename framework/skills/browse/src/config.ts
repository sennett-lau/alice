 ...
 *   2. git rev-parse --show-toplevel → projectDir/.tmp/nsmm/browse/
...
/**
 * Resolve the project-local nsmm state root.
 * Returns <git-root>/.tmp/nsmm/browse, or <cwd>/.tmp/nsmm/browse as fallback.
 * Used for state that must stay per-project (chromium profile, session files,
 * worktrees, sidebar queues).
 */
export function getBrowseStateRoot(): string {
  const root = getGitRoot() || process.cwd();
  return path.join(root, '.tmp', 'nsmm', 'browse');
}
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    // stateDir is <projectDir>/.tmp/nsmm/browse — walk up 3 to projectDir
    projectDir = path.dirname(path.dirname(path.dirname(stateDir)));
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.tmp', 'nsmm', 'browse');
...
/**
 * Create the .tmp/nsmm/browse/ state directory if it doesn't exist.
 * Throws with a clear message on permission errors.
 *
 * Note: .gitignore management is the project's responsibility — we expect
 * .tmp/ to already be ignored at the project root.
 */
