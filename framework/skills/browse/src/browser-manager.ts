...
import { getBrowseStateRoot } from './config';
...
export class BrowserManager {
  ...
  /**
   * Find the nsmm Chrome extension directory.
  ...
  async launch() {
    ...
    this.browser.on('disconnected', () => {
      ...
      console.error('[browse] Console/network logs flushed to .tmp/nsmm/browse/browse-*.log');
      process.exit(1);
    });
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the nsmm
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the nsmm extension directory for auto-loading
    ...
    const fs = require('fs');
    const path = require('path');
    const userDataDir = path.join(getBrowseStateRoot(), 'chromium-profile');
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('nsmm-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'nsmm-ctrl';
        topLine.style.cssText = `
          position: fixed; top: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
          background-size: 200% 100%;
          animation: nsmm-shimmer 3s linear infinite;
        ...
        style.textContent = `
          @keyframes nsmm-shimmer {
            0% { background-position: 200% 0; }
            100% { background-position: -200% 0; }
          }
          @media (prefers-reduced-motion: reduce) {
            #nsmm-ctrl { animation: none !important; }
          }
        `;
