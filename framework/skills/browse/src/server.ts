/**
 * nsmm browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.tmp/nsmm/browse/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.tmp/nsmm/browse/browse-{console,network,dialog}.log
...
import { resolveConfig, ensureStateDir, readVersionHash, getBrowseStateRoot } from './config';
...
function generateHelpText(): string {
  ...
  const lines = ['nsmm browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(getBrowseStateRoot(), 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', '_nsmm', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', '_nsmm', 'browse', 'dist', 'browse'),
  ];
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(getBrowseStateRoot(), 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(getBrowseStateRoot(), 'sidebar-agent-queue.jsonl');
  const stateParentDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(stateParentDir, { recursive: true });
...
async function shutdown() {
  ...
  const profileDir = path.join(getBrowseStateRoot(), 'chromium-profile');
